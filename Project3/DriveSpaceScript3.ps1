...
# It pulls the files from the specified drive (C:), sorts them in a descending order by length. Any files unaccesible are not displayed with the Error Action
function getFiles{
    Get-ChildItem c:\ -r -ErrorAction SilentlyContinue |sort -descending -property length| select -first 5 name, DirectoryName, @{Name="Size (GB)";Expression={[Math]::round($_.length / 1GB, 1)}}, @{Name="Size (MB)";Expression={[Math]::round($_.length / 1MB, 1)}} | FL | Out-String 
}
